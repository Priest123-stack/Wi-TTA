...
plt.subplots_adjust(bottom=0.08, right=0.8, hspace=0.3, wspace=0.25)  # 关键调整：right=0.8
...
for idx, algo in enumerate(original_algo_order):
    ...
    if not algo_data.empty:
        ...
        if row < 5:
            ...
            ax.set_xticklabels(custom_order, rotation=45, fontsize=8)
            ax.tick_params(axis='x', which='both', length=3)

        ax.set_ylim(0, cross_df["Std"].max() * 1.1)
        ax.grid(axis="y", linestyle="--", alpha=0.6)

    ax.set_title(f"{algo}", fontsize=10, pad=8)
    ax.set_ylabel("方差 (%)" if col == 0 else "", fontsize=9)

# 全局横坐标标签
fig.text(0.45, 0.03, "源域-目标域",
         ha='center', va='center',
...
plt.suptitle("不同TTA算法跨场景方差分析", y=0.995, fontsize=14)
plt.show()