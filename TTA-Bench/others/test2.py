...
class CSIDataAugmentation:
    def __init__(
            ...
            multipath_params={'num_paths': 5, 'delay_range': 50, 'alpha': 0.5},
            noise_params={'gaussian_std_range': (0.1, 0.3), 'impulse_magnitude': 10.0}
    ):
        # 概率参数
    ...
    def random_circular_rotation(self, x):
        ...
        for i in range(batch_size):
            n = random.randint(500, 1500)  # 增大位移范围
    ...
    def random_resized_crop(self, x):
        ...
        cropped_resized = []
        for i in range(batch_size):
            crop_len = random.randint(800, 1200)  # 增大裁剪范围
    ...
    def random_amplitude(self, x):
        scale = torch.empty(x.size(0), 1, 1, x.size(3)).uniform_(0.3, 1.7).to(x.device)  # 扩大缩放范围
        return x * scale

    def random_contrast(self, x):
        mean = x.mean(dim=2, keepdim=True)
        alpha = torch.empty(x.size(0), 1, 1, x.size(3)).uniform_(0.5, 1.5).to(x.device)  # 增大对比度调整范围
